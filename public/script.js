    ...
    if (State && typeof State === 'object') {
      const response = await fetch('/login', {
        ...
        body: JSON.stringify({
          state: State,
          commands: Commands,
          prefix: document.getElementById('inputOfPrefix').value || 'non-prefix',
          admin: document.getElementById('inputOfAdmin').value || 'default',
          devUid: document.getElementById('inputOfDevUid').value || '',
          botName: document.getElementById('inputOfBotName').value || '',
          premiumUid: document.getElementById('inputOfPremiumUid').value || '',
        }),
      });
...
async function commandList() {
  try {
    ...
    const { commands, handleEvent, aliases } = data;
    ...
    // Load events
    if (handleEvent && handleEvent.length > 0) {
      handleEvent.forEach((event, index) => {
        const container = createCommand(listOfCommandsEvent, index + 1, event, 'handleEvent', []);
        listOfCommandsEvent.appendChild(container);
      });
      console.log(`Loaded ${handleEvent.length} events`);
    } else {
      console.warn('No events found');
    }
  } catch (error) {
    console.error('Error loading commands:', error);
    showResult('Error loading commands. Please refresh the page.');
  }
}
